...
public class DropDownListView extends ListView implements OnScrollListener {
    ...
    private void initDropDownStyle() {
        ...
        headerLayout = (RelativeLayout) inflater.inflate(R.layout.ic_jpush_drop_down_list_header, this, false);
    ...
    public void onScroll(AbsListView view, int firstVisibleItem,
        ...
        if (isDropDownStyle) {
            if (currentScrollState == SCROLL_STATE_TOUCH_SCROLL && currentHeaderStatus != HEADER_STATUS_LOADING) {
                if (firstVisibleItem == 0 && actionMovePointY - actionDownPointY > 0) { //向下滑动
                    onDropDown();
                }
...
//			}
    ...
    public void setHeaderLayoutHide() {
        if (headerLayout != null && headerProgressBar != null) {
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.ic_jpush_drop_down_list_attr);
        isDropDownStyle = ta.getBoolean(R.styleable.ic_jpush_drop_down_list_attr_isDropDownStyle, false);
        ta.recycle();
    }
}
